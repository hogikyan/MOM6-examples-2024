...
!<PUBLICTYPE >
 type atmos_data_type
     ...
     real, pointer, dimension(:,:) :: u_ev     => NULL() ! u_evap
     real, pointer, dimension(:,:) :: v_ev     => NULL() ! v_evap
...
!quantities going from land+ice to atmos
   ...
   real, dimension(:,:),   pointer :: u_ev          =>null() ! surface air speed evaporation
   real, dimension(:,:),   pointer :: v_ev          =>null() ! surface air speed evaporation
...
allocate ( Atmos % u_ev(is:ie,js:je) )
allocate ( Atmos % v_ev(is:ie,js:je) )
...
101 FORMAT("CHECKSUM::",A16,a,'%',a," = ",Z20)
  ...
  write(outunit,100) ' atm%u_ev                  ', mpp_chksum(atm%u_ev                 )
  write(outunit,100) ' atm%v_ev                  ', mpp_chksum(atm%v_ev                 )
