...
subroutine surface_flux_1d (                                           &
     t_atm,     q_atm_in,   u_atm,     v_atm, u_ev, v_ev,    p_atm,     z_atm,    &
  ...
  real, dimension(size(t_atm(:))) ::                          &
       ...
       t_surf0,  t_surf1,  u_dif,     v_dif,  u_dif_ev, v_dif_ev,        &
       rho_drag, drag_t,    drag_m,   drag_q,    rho,      &
       q_atm,    q_surf0,  dw_atmdu,  dw_atmdv,  w_gust, w_atm_ev, &
       u_star_bad, b_star_bad, q_star_bad, &
       cd_m_bad, cd_t_bad, cd_q_bad
...
!     thv_surf= t_surf0                        ! surface virtual (potential) T -- just for testing tun off the q_surf
     ...
     u_dif_ev = u_surf - u_ev
     v_dif_ev = v_surf - v_ev
  endwhere
  ...
  if(alt_gustiness) then
     ...
     where(avail)
        ...
        w_atm_ev = sqrt(u_dif_ev*u_dif_ev + v_dif_ev*v_dif_ev + w_gust*w_gust)
        ! derivatives of surface wind w.r.t. atm. wind components
  ...
  if (ncar_ocean_flux .or. ncar_ocean_flux_orig) then
    call  ncar_ocean_fluxes (w_atm, th_atm, t_surf0, q_atm, q_surf0, z_atm, &
                             seawater, cd_m, cd_t, cd_q_bad, u_star, b_star     )
    call ncar_ocean_fluxes(w_atm_ev, th_atm, t_surf0, q_atm, q_surf0, z_atm, &
                     seawater, cd_m_bad, cd_t_bad, cd_q, u_star_bad, b_star_bad )
 end if
  ...
  where (avail)
     ...
     drag_t = cd_t * w_atm
     drag_q = cd_q * w_atm_ev  !cd  using w_ev
     drag_m = cd_m * w_atm
...
subroutine surface_flux_0d (                                                 &
     t_atm_0,     q_atm_0,      u_atm_0,     v_atm_0, u_ev_0, v_ev_0,  p_atm_0, z_atm_0,    &
  ...
  real, dimension(1) :: &
       t_atm,     q_atm,      u_atm,     v_atm,  u_ev , v_ev ,    &
       p_atm,     z_atm,      t_ca,                          &
  ...
  call surface_flux_1d (                                                 &
       t_atm,     q_atm,      u_atm,     v_atm, u_ev, v_ev,   p_atm,     z_atm,    &
       p_surf,    t_surf,     t_ca,      q_surf,                         &
       u_surf,    v_surf,                                                &
       rough_mom, rough_heat, rough_moist, rough_scale, gust,            &
       flux_t, flux_q, flux_r, flux_u, flux_v,                           &
...
subroutine surface_flux_2d (                                           &
  ...
  do j = 1, size(t_atm,2)
     call surface_flux_1d (                                           &
          t_atm(:,j),     q_atm_in(:,j),   u_atm(:,j),     v_atm(:,j), u_ev(:,j), v_ev(:,j),    p_atm(:,j),     z_atm(:,j),    &
