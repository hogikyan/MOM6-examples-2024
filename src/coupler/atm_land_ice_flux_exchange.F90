...
contains
  ...
  subroutine atm_land_ice_flux_exchange_init(Time, Atm, Land, Ice, atmos_ice_boundary, land_ice_atmos_boundary, &
    ...
    allocate( land_ice_atmos_boundary%u_ev(is:ie,js:je) )  ! akh
    allocate( land_ice_atmos_boundary%v_ev(is:ie,js:je) )  ! akh
    ...
    land_ice_atmos_boundary%u_ev=0.0  !akh
    land_ice_atmos_boundary%v_ev=0.0  !akh
  ...
  subroutine sfc_boundary_layer ( dt, Time, Atm, Land, Ice, Land_Ice_Atmos_Boundary )
    ...
    call data_override('ATM', 'u_ev', Atm%u_ev, Time)
    call data_override('ATM', 'v_ev', Atm%v_ev, Time)
    ...
    call put_to_xgrid (Atm%u_ev  , 'ATM', ex_u_ev  , xmap_sfc, remap_method=remap_method, complete=.false.)
    call put_to_xgrid (Atm%v_ev  , 'ATM', ex_v_ev  , xmap_sfc, remap_method=remap_method, complete=.false.)
    ...
    do l = 1, my_nblocks
       ...
       ie=block_end(l)
       call surface_flux (&
            ex_t_atm(is:ie), ex_tr_atm(is:ie,isphum),  ex_u_atm(is:ie), ex_v_atm(is:ie), ex_u_ev(is:ie), ex_v_ev(is:ie), &
            ex_p_atm(is:ie),  ex_z_atm(is:ie),  &
   ...
   do n = 1, ex_gas_fluxes%num_bcs  !{
    ...
    call get_from_xgrid (Land_Ice_Atmos_Boundary%u_ev,     'ATM', ex_u_ev     , xmap_sfc, complete=.false.) !AKH
    call get_from_xgrid (Land_Ice_Atmos_Boundary%v_ev,     'ATM', ex_v_ev     , xmap_sfc, complete=.true.) !akh
